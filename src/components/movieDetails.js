              ...
              <div className="flex gap-x-4 text-base items-center mobile-flex-moviedetail">
                <div className="flex w-[100px] relative gap-x-1 items-center h-4 text-base text-[#e4d804]">
                  <span className="text-white">
                    {getRating(onlyMovies) ? getRating(onlyMovies) : ""}
                  </span>
                  <Rating
                    value={rating}
                    className="hidden top-[0px] text-[10px]"
                    style={{ maxWidth: 80 }}
                    onChange={(e) => handleChange(e, onlyMovies.id)}
                  />
                </div>
                ...
                {!findItem(watchlist, onlyMovies.id) || !loggedin ? (
                  <button
                    onClick={() => {
                      const watchlistObj = {
                        buy_price:
                          (onlyMovies.media_type || mediaType) === "movie"
                            ? 10
                            : 12,
                        rent_price: 5.0,
                        movie_name: onlyMovies.name,
                        release_date: onlyMovies.release_date,
                        backdrop_path: onlyMovies.image,
                        image_url: onlyMovies.image,
                        movie_id: onlyMovies.id,
                        summary: onlyMovies.summary,
                      };
